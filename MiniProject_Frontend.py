...
class Movie:
    def __init__(self, root):
        ...
        MiniProject_Backend.connect()  # Call connect to ensure the table is created
        ...
        def clcdata():
            ...
            self.txtDirector.insert(END, "Enter Director Name")
        ...
        def adddata():
            if len(Movie_ID.get()) != 0:
                ...
                MovieList.insert(END, format_movie_data((Movie_ID.get(), Movie_Name.get(), Release_Date.get(), Director.get(), Cast.get(), Budget.get(), Duration.get(), Rating.get())))
                disdata()
        ...
        def disdata():
            ...
            for row in MiniProject_Backend.ViewMovieData():
                formatted_movie = format_movie_data(row[1:])
                for line in formatted_movie.split('\n'):
                    MovieList.insert(END, line)

        def format_movie_data(movie):
            formatted_details = ""
            details = {
                "Movie ID": movie[0]  if movie[0] else "Unknown" ,
                "Movie Name": movie[1]  if movie[1] else "Unknown",
                "Release Date": movie[2]  if movie[2] else "Unknown",
                "Director": movie[3] if movie[3] else "Unknown",
                "Cast": movie[4] if movie[4] else "Unknown",
                "Budget": movie[5] if movie[5] else "Unknown",
                "Duration": movie[6] if movie[6] else "Unknown",
                "Rating": movie[7] if movie[7] else "Unknown"
            }
            for label, value in details.items():
                formatted_details += f"{label}: {value}\n"
        ...
        self.TFrame = Label(TFrame, font=('Arial', 51, 'bold'), text="ONLINE MOVIE CART SYSTEM", bg="black", fg="orange")
        self.TFrame.grid()
        ...
        self.txtMovie_Name.insert(END, "Enter Movie Name")
        self.txtMovie_Name.bind("<FocusIn>", lambda event: self.txtMovie_Name.delete(0, END))
        ...
        self.txtRelease_Date.insert(END, "Enter Release Date")
        self.txtRelease_Date.bind("<FocusIn>", lambda event: self.txtRelease_Date.delete(0, END))
        ...
        self.txtDirector.insert(END, "Enter Director Name")
        self.txtDirector.bind("<FocusIn>", lambda event: self.txtDirector.delete(0, END))
        ...
        MovieList = Listbox(DFrameR, width=41, height=16, font=('Arial', 12, 'bold'), bg="black", fg="white", xscrollcommand=sb.set)
        MovieList.bind('<<ListboxSelect>>', movierec)
        MovieList.grid(row=0, column=0, padx=8)
        sb.config(command=MovieList.xview)
